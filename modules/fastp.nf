...
if counter != 0:
    sys.exit(1)"""])
    promise([FOR_ALL("f", ITER("*_fastp.json"), {f -> IF_THEN(EMPTY_FILE(f), "exit 1")}), FOR_ALL("f", ITER("*.trimmed.fastq"), {f -> IF_THEN(EMPTY_FILE(f), "exit 1")}), """#!/bin/python3
import json
import sys
import os
...
with open(file) as log:
        ...
        sys.exit(1 if (reads_before - reads_after) / reads_before > 0.95 else 0)""", """#!/bin/python3
import sys
import os
